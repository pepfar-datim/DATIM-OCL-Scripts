...
class DatimSyncMERIndicators(DatimBase):
    ...
    # Filenames
    old_dhis2_export_filename = 'old_dhis2_MERIndicators_export_raw.json'
    new_dhis2_export_filename = 'new_dhis2_MERIndicators_export_raw.json'
    converted_dhis2_export_filename = 'new_dhis2_MERIndicators_export_converted.json'
    ...
    def saveDhis2MERIndicatorsToFile(self, str_active_dataset_ids='', outputfile=''):
        url_dhis2_export = self.dhis2env + '/api/dataElements.xml?fields=id,code,name,shortName,lastUpdated,description,dataSets[id,name,shortName,lastUpdated],categoryCombo[id,code,name,lastUpdated,created,categoryOptionCombos[id,code,name,lastUpdated,created]]&paging=false&filter=dataSets.id:in:[' + str_active_dataset_ids + ']'
        if self.verbosity:
            self.log('DHIS2 MERIndicators Assessment Types Request URL:', url_dhis2_export)
    ...
    def dhis2oj_MERIndicators(self, inputfile='', outputfile='', MERIndicators_collections=None):
        ...
        with open(self.attachAbsolutePath(inputfile), "rb") as ifile,\
            ...
            for de in new_MERIndicators['dataElements']:
                concept_id = de['code']
                url = '/orgs/PEPFAR/sources/MERIndicators/concepts/' + concept_id + '/'
                c = {
                    'type':'Concept',
                    'id':concept_id,
                ...
                for deg in de['dataElementGroups']:
                    ...
                    target_url = '/orgs/PEPFAR/sources/MERIndicators/concepts/' + concept_id + '/'
                    ...
                    r = {
                        ...
                        'data':{"expressions": ['/orgs/PEPFAR/sources/MERIndicators/concepts/' + concept_id + '/']}
                    }
                    output[url] = r
                    num_references += 1
    ...
    def run(self):
        ...
        # STEP 1: Fetch OCL Collections for MERIndicators Assessment Types
        # Collections that have 'MERIndicators' in the name, __datim_sync==true, and external_id not empty
        if self.verbosity:
            self.log('**** STEP 1 of 13: Fetch OCL Collections for MERIndicators Assessment Types')
        if not self.runoffline:
            ...
            MERIndicators_collections = self.getOclRepositories(url=url_MERIndicators_collections, key_field='external_id')
            with open(self.attachAbsolutePath(self.MERIndicators_collections_filename), 'wb') as ofile:
                ofile.write(json.dumps(MERIndicators_collections))
            if self.verbosity:
                self.log('Repositories retreived from OCL and stored in memory:', len(MERIndicators_collections))
            ...
            with open(self.attachAbsolutePath(self.MERIndicators_collections_filename), 'rb') as handle:
        ...
        if not runoffline:
            content_length = self.saveDhis2MERIndicatorsToFile(
        ...
        self.dhis2oj_MERIndicators(inputfile=self.new_dhis2_export_filename,
                          outputfile=self.converted_dhis2_export_filename,
                          MERIndicators_collections=MERIndicators_collections)
        ...
        for ocl_export_def_key in self.OCL_EXPORT_DEFS:
            ...
            with open(self.attachAbsolutePath(export_def['jsonfilename']), 'rb') as ifile, open(
                ...
                ocl_MERIndicators_export = json.load(ifile)
                ocl_MERIndicators_export_clean = {}

                # Iterate through concepts, clean, then write
                for c in ocl_MERIndicators_export['concepts']:
                    url = c['url']
                    # Remove core fields
                    ...
                    ocl_MERIndicators_export_clean[url] = c

                # Iterate through mappings, clean, then write -- not used for MERIndicators assessment types
                for m in ocl_MERIndicators_export['mappings']:
                    url = m['url']
                    core_fields_to_remove = []
                    for f in self.MERIndicators_MAPPING_FIELDS_TO_REMOVE:
                        if f in m: del m[f]
                    ocl_MERIndicators_export_clean[url] = m
                ofile.write(json.dumps(ocl_MERIndicators_export_clean))
                if self.verbosity:
        ...
        diff = None
        with open(self.attachAbsolutePath(self.OCL_EXPORT_DEFS['MERIndicators_source']['jsoncleanfilename']), 'rb') as ocl_handle,\
        ...
        if self.import_test_mode:
            ...
            new_source_version_url = oclenv + '/orgs/PEPFAR/sources/MERIndicators/versions/'
            if self.verbosity:
            ...
            # TODO: MERIndicators collections
