"""
Class to synchronize FY20 DATIM MOH Alignment definitions between DHIS2 and OCL.
...
/orgs/PEPFAR/sources/DATIM-MOH-FY20/). Refer to init/importinit.py for more information
and to import the required starter content.

TODO: Implement new repo versioning model (e.g. FY20.v0)
...
class DatimSyncMohFy20(datimsync.DatimSync):
    """ Class to synchronize FY20 DATIM MOH Alignment between DHIS2 and OCL """
    ...
    REPO_ACTIVE_ATTR = datimconstants.DatimConstants.REPO_ACTIVE_ATTR_MOH_FY20

    # ID of the org and source in OCL
    DATIM_MOH_ORG_ID = 'PEPFAR'
    DATIM_MOH_SOURCE_ID = 'DATIM-MOH-FY20'
    ...
    # File names
    DATASET_REPOSITORIES_FILENAME = 'moh_fy20_ocl_dataset_repos_export.json'
    NEW_IMPORT_SCRIPT_FILENAME = 'moh_fy20_dhis2ocl_import_script.json'
    DHIS2_CONVERTED_EXPORT_FILENAME = 'moh_fy20_dhis2_converted_export.json'
    OCL_CLEANED_EXPORT_FILENAME = 'moh_fy20_ocl_cleaned_export.json'

    # Import batches
    IMPORT_BATCHES = [datimconstants.DatimConstants.IMPORT_BATCH_MOH_FY20]

    # DATIM DHIS2 Query Definitions
    DHIS2_QUERIES = datimconstants.DatimConstants.MOH_FY20_DHIS2_QUERIES

    # OCL Export Definitions
    OCL_EXPORT_DEFS = datimconstants.DatimConstants.MOH_FY20_OCL_EXPORT_DEFS
        ...
        with open(self.attach_absolute_data_path(dhis2filename_export_new), "rb") as input_file:
            ...
            for de in new_dhis2_export['dataElements']:
                ...
                de_concept = {
                    ...
                    'descriptions': None,
                    'extras': {'indicator_category_code': DatimSyncMohFy20.get_indicator_category_code(de_concept_id)},
                    'names': [
                        {
                            'name': de['name'],
                ...
                self.dhis2_diff[datimconstants.DatimConstants.IMPORT_BATCH_MOH_FY20][self.RESOURCE_TYPE_CONCEPT][
                ...
                for coc in de['categoryCombo']['categoryOptionCombos']:
                    ...
                    disag_classification = datimsyncmohhelper.DatimSyncMohHelper.get_disag_classification_fy20(
                    ...
                    disaggregate_concept_url = '/orgs/%s/sources/%s/concepts/%s/' % (
                        DatimSyncMohFy20.DATIM_MOH_ORG_ID, DatimSyncMohFy20.DATIM_MOH_SOURCE_ID,
                    ...
                    if disaggregate_concept_key not in self.dhis2_diff[
                            datimconstants.DatimConstants.IMPORT_BATCH_MOH_FY20][self.RESOURCE_TYPE_CONCEPT]:
                        ...
                        self.dhis2_diff[datimconstants.DatimConstants.IMPORT_BATCH_MOH_FY20][
                ...
                for dse in de['dataSetElements']:
                    ...
                    for disaggregate_concept_url in de_disag_concept_urls:
                        ...
                        if disaggregate_ref_key not in self.dhis2_diff[
                                datimconstants.DatimConstants.IMPORT_BATCH_MOH_FY20][self.RESOURCE_TYPE_CONCEPT_REF]:
                            self.dhis2_diff[datimconstants.DatimConstants.IMPORT_BATCH_MOH_FY20][
            ...
            self.vlog(1, 'DATIM-MOH FY20 DHIS2 export "%s" successfully transformed to %s data element concepts, '
